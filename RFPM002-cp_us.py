...
while(EscrBuffer):
    ...
    #oldtiempo = tiempo
    #tiempo = filter(allowed_forTime.__contains__, tiempo)
    #print(tiempo)
    #if (float(tiempo) >= float(oldtiempo)+10000) or (tiempo == '') or (float(tiempo)<float(oldtiempo)):
    #    print ("timestamp not valid")
    #    break
    #potencia = filter(allowed_forLevel.__contains__, potencia)
    #file.Escribir(tiempo, potencia)
    #if (float(potencia)>-70) & (float(potencia)<0 ) & (len(potencia)==6): # se descartan valores de potencia incoherentes
    #    muestras += 1
    #    print("0"+str(muestras)+','+ tiempo + ',' + potencia)
    #    file.Escribir(tiempo, potencia)
    #    VectorTimestamp.append(tiempo)
    #    meanpotencia = meanpotencia+pow(10,float(potencia)/10.0)
    #    
    #    if (muestras > 1) & (potenciaold<float(potencia)):
